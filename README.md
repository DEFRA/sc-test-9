...
```bash
docker build --target development --no-cache --tag sc-test-9:development .
```

Run:

```bash
docker run -p 3000:3000 sc-test-9:development
```
...
```bash
docker build --no-cache --tag sc-test-9 .
```

Run:

```bash
docker run -p 3000:3000 sc-test-9
```
